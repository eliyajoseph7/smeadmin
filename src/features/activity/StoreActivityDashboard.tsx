...
export const StoreActivityDashboard: React.FC = () => {
  const { storeId } = useParams<{ storeId: string }>();
  const navigate = useNavigate();
  const [data, setData] = useState<ActivityResponse | null>(null);
  const [loading, setLoading] = useState(true);
  const [timeRange, setTimeRange] = useState<'24h' | 'today' | 'week'>('24h');
  const [refreshing, setRefreshing] = useState(false);

  const loadStoreActivities = async (showLoader = true) => {
    if (!storeId) {
    ...
    try {
      ...
      switch (timeRange) {
        case 'today':
          response = await activityApiService.getStoreActivitiesToday(storeId);
          break;
        case 'week':
          response = await activityApiService.getStoreLastWeekActivities(storeId);
          break;
        default:
          response = await activityApiService.getStoreLast24HoursActivities(storeId);
      }
      
      setData(response);
    } catch (error: any) {
      console.error('Failed to load store activities:', error);
      toast.error(error.message || 'Failed to load store activities');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    await loadStoreActivities(false);
  };
  ...
  const getStoreName = () => {
    if (data?.activities && data.activities.length > 0) {
      return data.activities[0].storeName || 'Unknown Store';
    }
    return 'Store';
  };
  ...
  if (loading) {
    return (
      <>
        <PageHeader
          title={`Store Activity - ${getStoreName()}`}
          description="Monitor real-time activities for this specific store"
          icon={Store}
        />
        <div className="space-y-6 px-2 py-8">
          <div className="space-y-6">
            {/* Back Button and Controls Skeleton */}
      ...
      <div className="space-y-6 px-2 py-8">
        {data ? (
                ...
                <Card className="overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-200">
                    <h3 className="text-lg font-semibold text-gray-900">
                      Store Activities ({data.activities.length})
                    </h3>
                  </div>
                  ...
                  {data.activities.length === 0 ? (
                    <div className="p-8 text-center">
                      ...
                      <p className="text-gray-500">No activities found for this store in the selected time period</p>
                    </div>
                  ) : (
                          ...
                          <div className="flex items-start space-x-4">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="mt-1 text-xs text-gray-500">
                                Ref: {activity.entityReference} • {activity.serviceName}
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                <Card className="overflow-hidden">
                  ...
                  {data.activeUsers.length === 0 ? (
                    <div className="p-8 text-center">
                      ...
                      <p className="text-gray-500">No active users found for this store</p>
                    </div>
                  ) : (
                    <div className="divide-y divide-gray-100 max-h-96 overflow-y-auto">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-xs text-gray-400 mt-1">
                                {formatRelativeTime(user.lastActivityTime)}
                              </div>
                            </div>
                          </div>
                        </div>
            ...
            <Card className="p-4">
              <div className="flex items-center justify-center space-x-6 text-sm text-gray-600">
                <div className="flex items-center space-x-2">
                  ...
                  <span>
                    Period: {new Date(data.summary.periodStart).toLocaleDateString()} - {new Date(data.summary.periodEnd).toLocaleDateString()}
                  </span>
                </div>
                <div className="flex items-center space-x-2">
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-lg">No activity data available for this store</p>
